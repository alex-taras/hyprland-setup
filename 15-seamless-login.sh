#!/bin/bash
# seamless-login.sh - Seamless Plymouth to Hyprland transition via UWSM
# Based on Omarchy's approach - prevents terminal flashing between components
...
log "Compiling seamless-login helper..."
...
log "Creating seamless auto-login service..."
...
Conflicts=getty@tty1.service tuigreet.service greetd.service
...
log "Enabling seamless-login service..."
sudo systemctl daemon-reload
sudo systemctl enable seamless-login.service

log "Disabling conflicting services..."
sudo systemctl disable getty@tty1.service 2>/dev/null || true
sudo systemctl disable tuigreet.service 2>/dev/null || true
sudo systemctl disable greetd.service 2>/dev/null || true

log "Seamless login configured!"
warn "This replaces tuigreet with auto-login"
warn "Security relies on disk encryption + hyprlock"
warn "Reboot to test seamless Plymouth -> Hyprland transition"
